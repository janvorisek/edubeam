...
**Polygonal sections.** *Cross sections* tab → **Polygonal section** (or *Or define a polygonal shape* inside the *Add cross section* dialog) opens a shape editor. Start from a preset (rectangle, I, T, L, channel, rectangular/circular hollow, circle), then drag vertices, click an edge midpoint to insert a vertex, double-click a vertex to remove it, or type coordinates directly; holes are added as extra contours. The editor computes $A$, the centroid, $I_y$, $I_z$, $I_{yz}$, the principal moments $I_1$, $I_2$ with the principal-axis angle $\alpha$, the radii of gyration, and draws the ellipse of inertia. On save $A$, $I_y$ and $h$ are filled in from the shape (they show as read-only in the table – use the polygon button in the *Actions* column to edit the shape); the shear coefficient $k$ is still entered by hand. Section coordinates are local and right-handed with $x$ pointing out of the screen: $y$ points to the left, $z$ downwards.
